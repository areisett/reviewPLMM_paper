...
Genetic variants are often correlated with each other, meaning that univariate testing of genetic variants may result in false positives and biased effect estimates. This occurs when the variant being tested is correlated with other variants, so that the effects of these correlated variants are erroneously attributed to the variant in question. \anna{One method for analyzing data in the presence of correlated loci, or linkage disequilibrium (LD), is LD-pruning. In LD-pruning methods, SNPs are sequentially thinned, so that only SNPs correlated with each other below some desired threshold are used in modeling and testing procedures.} LD-pruning limits analysis to SNPs which are representative of a particular region. This may result in causal SNPs being removed from analysis, so that only SNPs in LD with the causal loci can be identified and their effects estimated. In addition, it has been shown that in the worst-case scenario, pruning algorithms may not select any representative SNPs in a particular region of the genome \cite{prive2018efficient}. However, LD-pruning does allow for the use of methods which assume uncorrelated loci such as \anna{BLAHHHHH: \url{https://www.nature.com/articles/srep27644/tables/1}}. In addition, LD-pruning may result in improved predictive accuracy \anna{because why}, which can be of particular benefit to polygenic risk score methods.  
...
The mechanism of environmental confounding is summarized in the directed acyclic graph displayed in figure \ref{fig:ps_env}. The undirected dashed line between population stratification and environmental exposure indicates that while we assume these elements are correlated, we do not assume any causal direction. Otherwise stated, we do not assume that environmental factors \anna{directly} influence population-specific allele frequencies, or vice versa, only that the two may be related.  \anna{often end up related for complex reasons... the environment and genetics have become associated over time for complicated reasons that we aren't going to get into... air quality isn't changing people's genetics, genetics aren't causing people to move to areas of better/worse air quality}
...
The genomic control method is a univariate testing procedure which modifies the Cochran-Armitage test for trend by a correction factor \pb{genomic control is not specific to trend tests; it can be used for any type of test statistic inflation}. This correction attempts to quantify and remove the effect of population structure on the distribution of the chi-square statistic using markers unrelated to the phenotype of interest and independent of the marker being tested for association \cite{devlin1999genomic, bacanu2000power, wang2009testing}. The genomic control method retains the disadvantages of univariate SNP testing and its power to detect true associations is likely to suffer in the presence of linkage disequilibrium \pb{reference?}.  \pb{This paragraph is a little too negative about genomic control...needs to be softened slightly}

Stratified analysis attempts to infer subject membership within a discrete number of nonoverlapping subpopulations. Subsequent analyses are conducted within each subpopulation, and the subpopulation-specific results may then be combined via meta-analysis \cite{pritchard1999use, pritchard2000association}. By partitioning the overall sample size in this manner, the structured association method is particularly vulnerable to a loss of power. Additionally, it relies on the assumption that these subpopulations are distinct and nonoveralapping. This assumption may not accurately reflect the characteristics of a particular sample, such as one consisting of admixed populations, or recent relatedness. Considered in the context of our air quality example, rather than the existence of two subpopulations with exposure to either good or bad air quality, there may also be individuals who fall somewhere between these two extremes \pb{seems useful to bring in idea of a gradient, or continuously varying relatedness, here}. By clustering those individuals within in one subpopulation or another we lose information about more high dimensional relatedness \pb{I think ``high dimensional relatedness'' is confusing/ambiguous here.  What do you mean by it?}.
...
\pb{Seems like we should explicitly include an intercept in all these models?}

\anna{typically Z is an incidence matrix... subpops q ... but not the only possibility}

To describe and compare PC-Lasso and LMM-Lasso, we consider an $n \times p$ matrix of genotype data $\boldsymbol{G}$ for $n$ subjects belonging to $q \le p$ subpopulations and $p$ SNPs, where each $G_{i,j} \in \{ 0, 1, 2 \}$ enumerates the number of allele copies subject $i$ has for SNP $j$ \pb{do we need to explicitly assume q discrete subpopulations here?} \anna{In order to show the equivalence of $u \sim N(0, \sigmagg \bK) \text{ and } Z\gamma \sim N(0, \sigmagg \bK)$ I think so, but maybe I can point out that this is the assumption the equivalence is arising from, but we don't' have to have discrete suppopulations in practice...\textbf{do this}}. We denote the standardized version \pb{need more explicit details on standardization} \anna{\textbf{do this} get details from Hayes 2009} of this genotype matrix as $\bX$. We assume that an $n \times 1$ vector of continuous phenotype values, $\by$, can be expressed as the sum of additive SNP effects, environmental confounding, and observational noise and consider the linear mixed model,
\begin{equation}
    \label{eqn:model}
...
Using the spectral decomposition of $\bK$ described in equation \ref{eqn:k}, we can gain further insight into the mechanism of this transformation. Note that $\bV^{-1}$ can be expressed using the following matrix factorization \pb{ONLY WORKS IF $\bX$ IS FULL RANK} \anna{I understand this on paper, but why does it work numerically? Even if $n > p$, svd(K) or eigen(K) gives a U that is $n \times n$... what are the vectors corresponding to the 0-eigenvalues? Toy example... PROVIDED U IS N X N}
\begin{align*}
...
\pb{should this paragraph move to next section?}
\anna{yes} Thus, given the nature of environmental confounding in our setting of interest, and in order to assess these methodological performance in terms of estimation accuracy, we simulate according to a fixed $\bgamma$ framework and using the following data-generating model
\begin{equation}
    \label{eqn:sim}
...
\pb{Need to describe how we select $\lambda$}

\anna{
\begin{itemize}
    \item Need details about the real data once it's run
    \item Need to fix facet labels
\end{itemize}
}
...
\begin{figure}[H]
    ...
    \caption{MSE for different data types in the presence and absence of environmental confounding. \anna{$\gamma$ structure is half-and-half-decreasing-heterogeneous, subpop size is large}}
    \label{fig:mse}
\end{figure}

\pb{Just FYI, it's hard to tell bias and var apart in black and white}

%------------------------------%
\subsection{LMM-Lasso outperforms PC-Lasso when simulated subpopulations are small}
%------------------------------%

\pb{I find ``small'' subpopulations unclear; how do you feel about ``fine'' and ``coarse'' structure?}
...
\anna{admixture simulated data sets do worse in the large subpops setting because their similarity matrix does not indicate as clearly who was exposed to one of e.g. 4 environmental exposures. the small subpopulations scenario more closely mimics gradations of environmental effects - what is usually simulated with a random draw from the MVN distn... we're simulating environmental heterogeneity in a way that allows us to examine the bias produced and these methods still do as well or better than pca}

\begin{figure}[H]
    \centering
    % \includegraphics[scale = 0.9]{figures/mse_diff_subpops}
     \includegraphics[scale = 0.9]{figures/mse_diff_subpops.png}
    \caption{Relative performance of LMM-Lasso, PC-Lasso in terms of estimation accuracy:  \anna{structureX is independent subpops, $\bgamma$ structure is dichotomous and heterogeneous, $\eta= 0.5, \xi$ is varying}}
...
\pb{We should discuss this section; to me, this section comes across as overly complicated and the point is not really coming across}
\anna{homogeneous reflects groups that are genetically similar have more similar environmental exposures, heterogeneous says this is not the case. For example... negative lung exposures that are geographic make more sense as a homogeneous exposure but something more behavior-based like smoking behaviors are more plausibly be heterogenous: e.g. smoking in China vs. France - similar behavior, different genetically}
...
\begin{figure}[H]
    ...
    \caption{Relative performance of LMM-Lasso, PC-Lasso for different subpopulation and $\bgamma$ structures: \anna{structureX is independent subpops, $\eta= 0.5, \xi = 0.8$}}
...
\anna{Fix factor label greek letters - R update broke some things}
\begin{figure}[H]
    \centering
    \includegraphics[scale = 1]{figures/eta_beta_hat.png}
     \caption{Caption goes here \anna{$\xi = 0.5$, structureX = independent subpops, structureGamma is dichotomous heterogeneous, large blocks}}
...
\begin{figure}[H]
    ...
    \caption{Caption goes here \anna{$\eta = 0.5, \xi = 0.8$, structureX = independent subpops, structureGamma is dichotomous heterogeneous, large blocks}}
    \label{fig:time}
\end{figure}
