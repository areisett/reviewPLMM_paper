...
\anna{REPLACE FIGURES WITH RESULTS FROM PLMM-LASSO ONCE FINISHED}
...
LMM-Lasso-Rakitsh and LMM-Lasso-ggmix achieve nearly identical MSE for $\bbeta$ in many scenarios, particularly where $n << p$, as shown in the top panel of Figure \ref{fig:eta_beta_mse}. When $n \ge p$ the estimation performance of LMM-ggmix becomes erratic, and in some cases worse than that of LMM-Lasso-Rakitsch. This is most dramatic in the presence of substantial environmental environmental confounding, $\xi = 0.8$, and $n = \{1200, 1400\}$. LMM-Lasso-ggmix achieves a moderate improvement in the estimation of $\eta$, particularly in the presence of lower $\eta$ values and where $n << p$, as shown in the middle panel of Figure \ref{fig:eta_beta_mse}. Interestingly, these improved $\eta$ estimates do not correspond to smaller $\bbeta$ MSE for LMM-Lasso-ggmix. When $n \ge 1000$, LMM-Lasso-ggmix's estimation of $\eta$ in suffers. When $n \ge 1000$ and in the presence of substantial environmental confounding, $\xi = 0.8$,  LMM-Lasso-ggmix's is hindered to the point that LMM-Lasso-Rakitsch estimates $\eta$ more accurately. 

Overall these results show that LMM-Lasso Rakitsch outperforms LMM-Lasso-ggmix in estimating $\bbeta$ and $\eta$ where $n \ge p$, likely due to the ill-conditioning of the RRM in these settings \anna{cite French paper (?)}. 
...
When $n < p$, LMM-Lasso-ggmix estimates $\bbeta$ as well as LMM-Lasso-Rakitsch, and $\eta$ better than LMM-Lasso-Rakitsch. However, in this setting the computational time LMM-Lasso-ggmix reqiures may be prohibitive for practical applications. The bottom panel of Figure \ref{fig:eta_beta_mse} shows the computing time required for each method in the various simulation settings.

\anna{ below in progress}

Unfortunately, the settings where ggmix estimates things well coincide with excessive times and when it works well in terms of time, it estimates poorly 

Despite its comparable performance in accurately estimating $\bbetaHat$, LMM-Lasso-ggmix takes requires an average of \anna{73.4} times the computing time of LMM-Lasso-Rakitsch. Thus, LMM-Lasso-Rakitsch seems to be a more attractive and scalable method than LMM-Lasso-ggmix.
...
\anna{add some info/figure on convergence and number of iterations for lmm-lasso vs ggmix}
