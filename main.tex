...
which can range from 0 (no confounding) to 1 (entirely confounding). In the case where $\bZ \bgamma$ is entirely independent of population structure, $\bZ \bgamma \in \mathcal{N}(\boldsymbol{P}_{\bX})$ and $\bX \boldsymbol{\tau} = \boldsymbol{0}$ so that $\mathbb{E}(\bbetaHat^*) = \bbeta$. In this scenario, the SNP-phenotype associations will be unbiased, but the residual variance will be increased. Although we focus our attention on accurately estimating fixed SNP effects, we note that this result could lead to substantial bias in narrow-sense heritability estimates. \pb{Hmm...could this sentence be moved somewhere else?  I feel that narrow-sense heritability deserves a paragraph somewhere}
