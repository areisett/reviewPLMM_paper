...
Whether or not such methods are effective at reducing confounding effects depends upon whether population structure can be accurately inferred, and if this is a good proxy for environmental effects. In the presence of unobserved heterogeneous environmental effects which are not well-approximated by inferred population structure, these effects will not confound SNP-phenotype associations. This is due to the absence of a causal pathway between population stratification and environment, as depicted in Figure \ref{fig:ps_env_block2}. In such a scenario, failing to account for unobserved environmental effects will result in greater phenotypic variability \anna{and as a result, greater SE in beta-hats... see causal notes}, however, it will not bias SNP estimates \anna{cite something from causal inference here}. We will return to this idea and its mathematical justification in section \ref{sec:methods}. 
