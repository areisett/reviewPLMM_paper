...
New statistical methods to control for the effects of confounding due to population stratification have proliferated as computational advances have allowed for the analysis of genetic data from large cohorts. Many of these methods attempt to block the causal pathway between population structure and observed SNP data. This is illustrated by the blocked arrow in Figure \ref{fig:ps_env_block}. These methods do not directly address the environmental component of confounding due to population structure. Nevertheless, they may be effective if they are able to indirectly block the causal pathway between the environment and observed SNPs. 

\anna{make better slashed arrows / DAGs}
...
Whether or not such methods are effective at reducing confounding effects depends upon whether population structure can be accurately inferred, and if this is a good proxy for environmental effects. In the presence of unobserved heterogeneous environmental effects which are not well-approximated by inferred population structure, these effects will not confound SNP-phenotype associations. This is due to the absence of a causal pathway between population stratification and environment, as depicted in Figure \ref{fig:ps_env_block2}. In such a scenario, failing to account for unobserved environmental effects will result in greater phenotypic variability \anna{and as a result, greater SE in beta-hats... if we're talking about gls}, however, it will not bias SNP estimates \anna{cite something from causal inference here}. We will return to this idea and its mathematical justification in section \ref{sec:methods}. 
...
>=latex',
circ/.style={draw, shape=circle, node distance=1cm, line width=1pt}]%Define the arrow type and style for circled nodes
\draw[dashed, red] (0,1) node[left] (P) {Population Stratification} --node {X} (2,1) node[right] (E) {Environment};
\draw[->] (0,0) node[left] (X) {Observed SNPs} -- (2,0) node[right] (Y) {Phenotype}; %Line between X and Y
\draw[->] (P) -- (X) ; 
\draw[->] (E)   -- (Y) ; 
% \filldraw[color=red!60, fill=none, very thick] (0,1) ellipse (4.5 and 0.35);
