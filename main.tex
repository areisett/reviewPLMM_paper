...
$$\bgamma \sim N\left(\boldsymbol{0}, \frac{\sigmagg}{p} \bI_p \right).$$  Typically, $\bZ$ is not observed directly, and a common convention is to assume $\bZ = \bX$ \citep{wang2018multiplex, lippert2011fast, yang2014advantages}. Then, rather than assuming anything about $\bgamma$ directly, let $\bu=\bX\bgamma$ represent the random effect and assume $\bu \sim N(\boldsymbol{0}, \sigmagg \bK)$. This results in the LMM regression model
\begin{equation}
    \label{eqn:lmm_reg}
