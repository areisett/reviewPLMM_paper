...
We begin by comparing the ordinary Lasso, PC-Lasso, and LMM-Lasso as the level of environmental confounding is varied. Estimation accuracy for Lasso, PC-Lasso, and LMM-Lasso was evaluated based on mean squared error (MSE) and broken down into bias and variance components \pb{let's just report MSE (and maybe MSPE too); breakdown blurs ``bias'' concepts a bit}. Figure \ref{fig:mse} presents these results for all three data types in a 1:1 signal to noise (SNR) setting, where $\eta = 0.5$. The top row of Figure \ref{fig:mse} shows that when $\xi = 0$ (i.e., no environmental confounding), Lasso and LMM-Lasso perform comparably. PC-Lasso results in a slight inflation in MSE compared to the other methods in the simulated data sets due to the fact that 10 additional predictors (PCs) have been added to the model, but none of them actually explain any variability in the outcome. 
...
Given the superior performance of PC-Lasso and LMM-Lasso compared to the ordinary Lasso in the presence of environmental confounding, we next compare these methods in the presence of fine and coarse subpopulation structures \pb{as defined in section....}. Phenotypes were simulated using a 1:1 SNR, $\eta = 0.5$, and varying levels of environmental confounding, $\xi \in \{0.2, 0.5,0.8\}$. 

\begin{figure}[H]
    \centering
    % \includegraphics[scale = 0.9]{figures/mse_diff_subpops}
     \includegraphics[width=0.8\textwidth]{figures/mse_diff_subpops.png}
    \caption{Relative performance of LMM-Lasso, PC-Lasso for fine and coarse subpopulation structures and varying levels of environmental confounding with Independent Subpopulations data, $\eta = 0.5$, and dichotomous-discordant environmental effects. \pb{This figure could maybe be shorter -- takes up a lot of vertical room that isn't really necessary}}
...
In addition to evaluating PC-Lasso and LMM-Lasso in the presence of coarse and fine subpopulation structure, we also consider various patterns of environmental effects. Figure \ref{fig:big_small_gamma} shows the difference in MSE (PC-Lasso - LMM-Lasso) broken down by both subpopulation and \pb{``coefficient structure'' is vague; can we describe this better?} coefficient structure. Results are shown for phenotypes generated with $\eta = 0.5$, $\xi = 0.8$, and Independent Subpopulations.

\pb{What do you think about dropping the exponential/dichotomous thing and just going with dichotomous throughout?  Doesn't really seem super important.  Well, we can just keep it in mind and if we need to cut down on space / figures, we can think about it.}
...
\pb{Not sure I love this figure, as it emphasizes exp vs dichotomous, which isn't really very interesting.  Can we re-panel it so that it emphasizes discordant vs concordant?}
...
LMM-Lasso-Rakitsh and LMM-Lasso-ggmix achieve nearly identical MSE for $\bbeta$ in many scenarios, particularly where $n << p$ \pb{you want $n \ll p$, not $n << p$; this occurs a bunch}, as shown in the top panel of Figure \ref{fig:eta_beta_mse}. However, when $n \ge p$ the estimation performance of LMM-ggmix becomes erratic, and in some cases worse than that of LMM-Lasso-Rakitsch. This is most dramatic in the presence of substantial environmental environmental confounding, $\xi = 0.8$, and $n = \{1200, 1400\}$. 
