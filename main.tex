...
The ordinary lasso is clearly most susceptible to selecting non-causal SNPs due to their association with population structure. This corresponds to the simulation results shown in Section~\ref{sec:sel_bias}, though in Figure \ref{fig:gw_pc1_corr} we more plainly see that this bias persists across the $\lambda$ path. Likewise, PC-lasso most effectively controls this type of selection bias. This is to be expected given that we are assessing the population-informative nature of incorrectly selected SNPs via correlation with PC1, which is explicitly adjusted for in the PC-lasso method. LMM-lasso yields a substantial improvement compared to the ordinary lasso in controlling this selection bias. It should be noted that the Empirical data's RRM in Figure~\ref{fig:empirical} shows that its structure is predominantly coarse. Given our results in Section~\ref{sec:sim-fine-coarse}, we might expect a still more substantial improvement over the ordinary lasso in the analysis of more finely structured genome-wide data.
...
\anna{Add discussion of genome-wide results}
