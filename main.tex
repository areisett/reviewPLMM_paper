...
%------------------------------%
Given the superior performance of PC-Lasso and LMM-Lasso compared to the naive lasso in the presence of environmental confounding, we next compared these methods in the presence of two forms of simulated population structure: (1) relatedness characterized by a small number of large subpopulations \anna{with moderate relatedness (?)} to model large-scale environmental heterogeneity or population stratification, similar to that described in our motivating air qualitty example, and (2) a large number of small, closely related subpopulations intended to approximate finer grained environmental heterogeneity, such as household exposures or behavioral norms. Figure \ref{fig:kinship} illustrates the simulated kinship structures in these scenarios, \anna{as well as that present in the empirical SNP data}. We note that we show results for simulated data generated such that subpopulations were of equal size. Those of unequal size were also considered, but did not result in substantively different results (not shown - could be in supplement).
...
\end{subfigure}
\caption{\anna{obviously these figures need to be tweaked:sizes, uniform scales} \\ \anna{Should kinship in the first figure be lower - more moderate?}}
\label{fig:kinship}
\end{figure}

Pseudophenotypes for these simulations were again generated using a 1:1 SNR, $\eta = 0.5$, and in the presence of moderate environmental confounding, $\xi = 0.5$. \anna{Due to constraints within the BNPSD package and the nature of the admixed population model they use (???), only the independent subpopulations method was able to simulate a large number of small subpopulations. For this reason, the presented simulated results all correspond to this simulation method from the bnpsd package.} Figure \ref{fig:big_vs_small} summarizes the results of these simulations with the y-axis displaying the difference in MSE (PC-Lasso - LMM-Lasso) for each simulation and for the various subpopulation structures. The dashed horizontal line at 0 indicates no difference in the performance of PC-Lasso and LMM-Lasso, while differences greater than 0 correspond to smaller MSE for LMM-Lasso, and thus superior estimation accuracy. In the presence of a small number of large subpopulations, there is some overlap in the estimation accuracy of PC-Lasso and LMM-Lasso, as evidenced by the corresponding box straddling the 0 line. However, the majority of the box falls below 0, indicating the superior performance of PC-Lasso in many of the simulations. IN the presence of many small subpopulations, however, the entire box and error bars remain above the 0 line, indicating the clear superior estimation accuracy of LMM-Lasso in this context. \cite{hoffman2013correcting} showed that principal component adjustment methods represent a low dimensional approximation to linear mixed models. As such, it makes sense that PC-Lasso, which corrects for population structure using a low dimensional set of surrogate variables, would perform well in the large subpopulation setting, but fall short in the presence of many small subgroups. \anna{Additionally, its performance in this context may also me augmented by the fact that the subpopulations are more discretized  easily distinguishable based on their observed genetic data in the independent subpopulation simulated data. Although small subpopulations are unable to be simulated in the BNPSD admixed subpopulation setting, the corresponding results for the large subgroups in the admixed simulated data show much more comparable results between PC-Lasso and LMM-Lasso. - do this}. \anna{Vanderwas?}


\anna{Should Vanderwas simulated data be here?}
\begin{figure}[H]
    \centering
...
In addition to considering the effect of population structure, or forms of $\bZ$, on these methods' estimation accuracy, we also evaluated the effect of varying forms of $\bgamma$, the subpopulation-specific effect sizes. Our initial thinking was that structures of $\bgamma$ that can be well-approximated by a linear combination of the observed genetic data would be well-approximated and corrected for by PC-Lasso, whereas LMM-Lasso may be more able to correct for subpopulation-specific effects that do not lend themselves to linear approximations. To that end we investigated four distinct $\bgamma$ structures: linear homogeneous, linear heterogeneous, nonlinear homogeneous, and nonlinear heterogeneous, \anna{names should change... half and half, not exponential is the better nonlinear setting}. In our naming convention, linear/nonlinear refers to whether the coefficients follow a linear pattern versus a nonlinear form, while homogeneous/heterogeneous indicates whether coefficients have the same, or differing signs, respectively. For example, (1, -2, 3, -4) is an example of a linear-heterogeneous structure, whereas (2, 2, 1, 1) is a nonlinear-homogeneous form. Figure \ref{fig:gamma_structures} displays a visual summary of these coefficient structures. \\


\anna{put half and half in this plot instead of exponential? In addition?}
\begin{figure}[H]
    \centering
...
In section 3 briefly mentioned the existence of to LMM-Lasso variants: LMM-Lasso-Rakitsch and LMM-Lasso-ggmix. The LMM-Lasso results shown so far are those of LMM-Lasso-Rakitsch, though they are representative of LMM-Lasso-ggmix. The primary purpose of this review is to compare LMM-Lasso to other penalized regression methods which aim to correct for population structure and environmental heterogeneity. However, we also want to explain why we used LMM-Lasso-Rakitsch, and provide guidance for potential users of either method. 

LMM-Lasso-Rakitsch estimates the variance components once, under the assumption of a null model. LMM-Lasso-ggmix seeks to improve upon this by re-estimating the variance components and $\bbeta$ iteratively. Although this underlying idea seems reasonable, we have been unable to find a scenario where LMM-Lasso-ggmix outperforms the LMM-Lasso-Rakitsch in estimation accuracy of $\bbeta$ or $\eta$. 
...
In addition, we have found that LMM-Lasso-ggmix performs poorly in a low-dimensional setting due to the ill-conditioning of the RRM. Given its intended use for the analysis of high-dimensional data, this fact could be of little consequence to practitioners. However, although its estimation accuracy in high dimensions is on par with that of LMM-Lasso-Rakitsch, \anna{as shown in two previous mse figs}, this is also a setting in which the computational time required may be prohibitive for practical applications. Figure \anna{time fig} shows a ratio of the computing time required for LMM-Lasso-ggmix compared to that of LMM-Lasso-Rakitsch, where the horizontal dashed line at 1 indicates both methods require the same computing time. In high dimenions, where LMM-Lasso-ggmix performs well, it is between \anna{something and something else} times slower than LMM-Lasso-Rakitsch. Thus, LMM-Lasso-Rakitsch seems to be a more attractive and scalable method. We have implemented the LMM-Lasso-Rakitsch method using the equivalent but more interpretable $\eta$ paramaterization in the \texttt{R} package, \texttt{penalizedLMM - or something more fun}, available on the author's github page: \url{https://github.com/areisett/penalizedLMM}.
