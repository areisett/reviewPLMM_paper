...
%------------------------------%
\subsection{\anna{Genome-wide analysis}}
%------------------------------%

To illustrate the statistical properties demonstrated by our simulations on a more realistic scale, we present an analysis of the full Empirical genomic data ($n=588, p=526,266$) from \citet{larkin2015objectives} with a synthetic phenotype as a pseudo-real data case study. We generate the synthetic phenotype as in \eqref{eqn:sim} using 50,000 causal SNPs, a 1:1 SNR ($\eta = 0.5$), and substantial environmental effects ($\xi = 0.8$).
...
%------------------------------%
\subsection{\anna{LMM-lasso outperforms lasso when analyzing genome-wide data}}
%------------------------------%
...
\label{fig:gw_prc}
\end{figure}
\anna{should the y-axes on this and the below fig match? i.e. both be number of selected snps, or both be lambda indices?}\\
\anna{Try number of snps selected for x-axis, keep y-axis as precision}\\
\anna{Also why is pc-lasso doing so terribly? \textbf{depends on how you're defining precision... selects a lot of true snps but often they have the wrong sign}}\\
...
\end{figure}
\anna{Should y-axis here be \# of selected snps to match above fig?}
