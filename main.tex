...
%Univariate approaches attempt to estimate the marginal effect of a SNP on the phenotype and assess the statistical significance of SNP-phenotype associations while controlling for population structure by modeling it as a random effect using a genetic similarity matrix
...
\anna{More discussion of nonlinear models. In particular, is logistic regression practical? - threshold methods, probit regression by lippert}
...
\subsection{Software}
\label{sec:software}
\anna{More discussion of software. Add a small section? R-based Software for lasso, pc-lasso, ggmix, plmm, PLINK functions, KING, snpRelate. Remove other mentions of software in other places and put them here}
...
\anna{\textbf{penalty parameter selection; also a tough one. We need to say more, but not a great deal is known.}}

The nature of the simulations and analyses in this work have allowed us to select penalty parameters based on the true number of causal features in a model, which allows for fair comparison across methods. In a real data scenario, the true number of causal SNPs would not be known a priori. As such, appropriate methods for penalty parameter selection in penalized LMMs are worth additional discussion. As of yet there is no clear consensus on the optimal way to select a penalty parameter for penalized LMMs. 

Several authors have used the Bayesian information criterion (BIC) for variable selection in mixed models \cite{bondell2010joint, schelldorfer2011estimation}. However it has been shown that the Akaike information criterion (AIC) and BIC may perform poorly in the high-dimensional setting \citep{fan2013tuning}. The high-dimensional BIC introduced by \citet{fan2013tuning} penalizes model parameters more severely in an attempt to perform better in high dimensional settings. \citep{bhatnagar2020simultaneous} employs this high-dimensional BIC as its default method to select $\lambda$.

Subsampling approaches, such as CV and stability selection, are another common class of variable selection methods often preferred to information criterion-based approaches. \anna{cite? It seems like you see CV done way more often...} \citet{Rakitsch2012} used stability selection to evaluate the significance of individual SNPs \citep{meinshausen2010stability}. \citet{guo2019combining} used a combination of both methods for feature selection, first selecting several $\lambda$ values via CV, and then using these penalty parameters for stability selection and final feature selection. 

As previously mentioned, however, the advisability of subsampling methods in the presence of correlated observations remains an open question \citep{roberts2017cross, jia2012preconditioning}. A rigorous assessment of penalty parameter selection in the context of penalized LMMs is beyond the scope of this work, but worthy of further investigation.

\anna{I don't think we should bring up the idea of CV/subsampling on original vs transformed data in this paper at all - I think it's just confusing without a lot of additional explanation.}
