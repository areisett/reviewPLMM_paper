...
\begin{equation}
    \label{eqn:ratio}
    ||\bX \boldsymbol{\tau}||_2^2 / ||\bZ \bgamma||_2^2,
\end{equation}
which we refer to as the Confounding Proportion (CP). CP can range from 0 (no confounding) to 1 (entirely confounded). In the extreme case where $\bZ \bgamma$ is entirely independent of population structure, $\bZ \bgamma \in \mathcal{N}(\boldsymbol{P}_{\bX})$ and $\bX \boldsymbol{\tau} = \boldsymbol{0}$ so that $\mathbb{E}(\bbetaHat^*) = \bbeta$. In this scenario, the SNP-phenotype associations will be unbiased, but the residual variance will be increased. 

In real data settings CP cannot be calculated since $\bZ \bgamma$ is not directly observed. In simulation settings, however, this ratio is a useful metric for quantifying the degree to which $\bZ \bgamma$ reflects population structure and has the potential to bias SNP-phenotype associations. We will use CP in our interpretation of the simulation results in Section \ref{sec:results}. Given the high-dimensional nature of our data, we approximate $(\bX^\T \bX)^{-1}$ using a low-rank approximation. We use this approach in order to ensure a unique solution.
...
\anna{HERITABILITY PARAGRAPH}\\
Although we focus our attention on accurately estimating fixed SNP effects, we note that the result shown in \ref{eqn:partition_e} could lead to substantial bias in narrow-sense heritability estimates. \pb{Hmm...could this sentence be moved somewhere else?  I feel that narrow-sense heritability deserves a paragraph somewhere} 
