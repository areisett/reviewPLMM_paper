...
Although cross-validation and information criterion-based methods are commonly used to select $\lambda$ values, in this context they have the potential to result in misleading comparisons across methods. Cross-validation may be inadvisable for LMMs, and in the presence of non-inedpendent samples as is the case here \anna{I know i have a citation for this somewhere FIND IT}. While information criterion methods such as BIC present another option for selecting $\lambda$, they may be unstable in high dimensions. In addition, because Lasso, PC-Lasso, and LMM-Lasso use different likelihoods, an information-criterion-based approach would not lend itself to comparable results across methods. 

1000 simulations were performed for each unique combination of parameters, datatype, subpopulation structure, and environmental effect structure. \anna{somewhere else to put this that makes more sense?}
...
Due to the differing sample sizes and subpopulation structures, a direct comparison of the Empirical CP with those from the simulated data sets is more complex... this increases with equal subpopulation sizes
...
\anna{\textbf{check this value for sampsizes of 588 for admixture and independent...maybe even diff subpop sizes? at least to pin down the issue - more comparable with same sampesize and subpop sizes but still different...}}
 ...
 \anna{correlation between columns of X..., mspe vs mse- for various reasons CP is a reasonable measure but not perfect...} 
 
 
\anna{with the same sample size and subpopulation structure as the vanderwas data, I can replicate the CP for the indepenent subpopulations data when simulating using a lower Fst value (as defined in the ochoa and storey papers: the mean inbreeding coefficient of the individuals’ local populations relative to their last common ancestral population. This FST naturally yields a useful pairwise FST between individuals.) So, this suggests that the greater improvement in the semi-empirical data is because there is less inbreeding (?????) still thinking about this...}
