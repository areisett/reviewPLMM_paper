...
This result also allows us to mathematically justify the statement from section \ref{sec:background}, that SNP-phenotype associations will be unbiased in the presence of environmental heterogeneity which does not correspond to population structure. We can quantify the proportion of the environmental effect which is in the column space of $\boldsymbol{P}_{\bX}$ \anna{and thus biases the $\bbeta$ estimates} using the ratio \anna{$||\bX \boldsymbol{\tau}||_2^2 / ||\bZ \bgamma||_2^2 \in [0, 1]$}. In the case where $\bZ \bgamma$ is entirely independent of population structure, $\bZ \bgamma \in \mathcal{N}(\boldsymbol{P}_{\bX})$ and $\bX \boldsymbol{\tau} = \boldsymbol{0}$ so that $\mathbb{E}(\bbetaHat^*) = \bbeta$. Thus the SNP-phenotype associations will be unbiased, but the residual variance will be increased. Although we focus our attention on accurately estimating fixed SNP effects, we note that this result could lead to substantial bias in narrow-sense heritability estimates. 

In practical applications $\bZ \bgamma$ is unlikely to be entirely orthogonal to population structure. However, the degree to which it reflects population structure that can be inferred from $\bX$, as quantified by \anna{$||\bX \boldsymbol{\tau}||_2^2 / ||\bZ \bgamma||_2^2$}, will impact the extent to which environmental heterogeneity \anna{has the potential to bias} SNP-phenotype associations. \anna{We will use this metric in Section \ref{sec:results}. Given the high-dimensional nature of our data, we will approximate $(\bX^\T \bX)^{-1}$ using a low-rank approximation. We use this approach as opposed to that of a generalized inverse in order to ensure a unique solution.}
