...
% As with multiple linear regression models, lasso penalized regression assume subjects are independent of each other with uncorrelated errors. In the presence of population structure or cryptic relatedness, however, samples are correlated, violating this assumption. If unaccounted for, this dependency can result in the identification of spurious associations. 

As with multiple linear regression models, lasso penalized regression assume subjects are independent of each other with uncorrelated errors. In the presence of population structure or cryptic relatedness, however, samples are correlated, violating this assumption. The presence of structure can adversely affect analyses in several ways. Firstly, allele frequency differences due to ancestry within a sample can be mistaken for biological signal if steps are not taken to mitigate this effect. One common solution, dividing a sample and performing ancestry-specific analyses, often results in a loss of statistical power from reduced sample sizes. Secondly, correlation among subjects within a sample, whether due to common ancestry or more recent relatedness, violates an assumption that many regression methods rely on, rendering these statistical methods invalid.
...
\end{equation}
where $|| \boldsymbol{\beta} ||_1 = \sum_{j=1}^p |\beta_j|$ denotes the $\ell_1$ norm of the regression coefficients, and $||\mathbf{x}||_2 = \sqrt{\sum_{i=1}^p x_i^2}$ the $\ell_2$ norm. The first component of the objective function is the linear regression loss function, which quantifies the overall model fit and captures how well the estimated parameters agree with the observed data. As in ordinary least squares (OLS) regression, the lasso assumes observations are independent. The second component is the $\ell_1$ penalty applied to the estimated coefficient values (but not the intercept, $\beta_0$), which discourages large, unrealistic parameter values. The tuning parameter, $\lambda$, controls the tradeoff between these two components. For sufficiently large values of $\lambda$ all elements of $\bbeta$ will be estimated to be 0. In the $p \ll n$ setting, if $\lam = 0$, the lasso solution coincides with that of OLS. The $\ell_1$ penalty of the lasso can be applied to nonlinear models such as Cox regression \citep{tibshirani1997lasso} and logistic regression \citep{shevade2003simple} by replacing the residual sum of squares with the appropriate negative log-likelihood function.
...
%------------------------------%
\label{sec:software}
\anna{Should this be here as a methods subsection or should it get its own section after discussion, before data availability?}
...
%------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\anna{Simulation results}} \label{sec:results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\anna{Genome-wide case study}} \label{sec:gw_cs}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%------------------------------%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

To illustrate the statistical properties demonstrated by our simulations on a more realistic genome-wide scale, we present an analysis of the full Empirical genomic data ($n=588, p=526,266$) from \citet{larkin2015objectives} and a synthetic phenotype as a pseudo-real data case study. We generate the synthetic phenotype as in \eqref{eqn:sim} using 50,000 causal SNPs, a 1:1 SNR ($\eta = 0.5$), and substantial environmental effects ($\xi = 0.8$). In order to compare all methods' performance fairly and completely, we present results for the entirety of each method's $\lambda$ path.

%------------------------------%
\subsection{\anna{LMM-lasso identifies causal SNPs with greater precision than lasso}}
%------------------------------%
\begin{figure}[H]
\centering
\includegraphics[scale = 0.6]{figures/genomewide_prc_p1=100.png}
\includegraphics[scale = 0.6]{figures/genomewide_prc_p1=500.png}
\includegraphics[scale = 0.6]{figures/genomewide_prc_p1=1000.png}
\caption{Precision values for each method and number of selected SNPs from the Empirical genome-wide analysis. Single replication case study with $\eta = 0.5, \xi = 0.8$.}
\label{fig:gw_prc}
\end{figure}

$$\text{Precision }= \frac{\#\{ \beta > 0 \cap \widehat{\beta} \ne 0  \}}{\#\{ \widehat{\beta} \ne 0 \}}$$


%------------------------------%
\subsection{\anna{PC-lasso and LMM-lasso increase estimation accuracy}}
%------------------------------%

\begin{figure}[H]
\centering
\includegraphics[scale = 0.6]{figures/genomewide_mse_p1=100.png}
\includegraphics[scale = 0.6]{figures/genomewide_mse_p1=500.png}
\includegraphics[scale = 0.6]{figures/genomewide_mse_p1=1000.png}
\caption{Pathwise MSE from the Empirical genome-wide analysis. Single replication case study with $\eta = 0.5, \xi = 0.8$.}
\label{fig:gw_mse}
\end{figure}

\noindent\anna{should the y-axes on this and the below fig match? i.e. both be number of selected snps, or both be lambda indices?}\\
...
\anna{rewrite to reflect that the x-axis is number of selected snps, not lambda indices. update results / discussion as necessary based on final p1 chosen} Figure \ref{fig:gw_pc1_corr} displays the mean absolute correlation of the non-causal SNPs selected for a particular $\lambda$ value with the first principal component (PC1) of the Empirical data for each method across the full $\lambda$ path. The corresponding shaded regions indicate the SE about the mean. Overall, these bounds are largest early on in each $\lambda$ path, where only a few SNPs are be selected, but narrow as $\lambda$ increases and more variables are allowed to enter the model.\anna{Do I need to explain why we are using correlation with the first PC as a proxy for MAF difference or is this obvious?} The absolute correlation with PC1 is used as a measure of how population-informative the selected SNPs are given the complex Empirical data structure compared to that in Section~\ref{sec:sel_bias}.

\begin{figure}[H]
\centering
\includegraphics[scale = 0.6]{figures/genomewide_pc1_corr_p1=100.png}
\includegraphics[scale = 0.6]{figures/genomewide_pc1_corr_p1=500.png}
\includegraphics[scale = 0.6]{figures/genomewide_pc1_corr_p1=1000.png}
\caption{Absolute correlation of selected, non-causal SNPs with the first principal component (PC1) of the data in Empirical genome-wide analysis. Plotted line is the mean absolute correlation of all incorrectly selected SNPs for a particular number of incorrectly selected SNPs, with shaded region indicating SE bounds. Single replication case study with $\eta = 0.5, \xi = 0.8$.}
\label{fig:gw_pc1_corr}
\end{figure}

The ordinary lasso is clearly most susceptible to selecting non-causal SNPs due to their association with population structure. This corresponds to the simulation results shown in Section~\ref{sec:sel_bias}, though in Figure \ref{fig:gw_pc1_corr} we more plainly see that this bias persists across the $\lambda$ path. Likewise, PC-lasso most effectively controls this type of selection bias. This is to be expected given that we are assessing the population-informative nature of incorrectly selected SNPs via correlation with PC1, which is explicitly adjusted for in the PC-lasso method. LMM-lasso yields a substantial improvement compared to the ordinary lasso in controlling this selection bias. 
...
\anna{\textbf{Additional genome-wide results based on other figures we decide to include.} The results of a genome-wide analysis of Empirical data agree with the findings of our simulation experiments, in particular that PC-lasso and LMM-lasso are able to substantially reduce selection bias compared to the ordinary lasso. Although PC-lasso was found to control this selection bias most effectively, it should be noted that because the Empirical data consists of three distinct ethnic groups, its RRM (Figure~\ref{fig:empirical}) demonstrates predominantly coarse population structure. Given our results in Section~\ref{sec:sim-fine-coarse}, we might expect LMM-lasso to provide greater improvement over the ordinary lasso in the analysis of more finely structured genome-wide data.}
